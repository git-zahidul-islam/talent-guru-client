...
const JobDetails = () => {
    ...
    const handleFormSubmit = async (e) => {
        e.preventDefault()
        if(user?.email === buyer?.email){return alert('you create this job')}
        const form = e.target;
        const jobId = _id;
        const price = parseFloat(form.price.value);
        if (price < parseFloat(min_price)){return alert("offer more or at last equal to min price")}
        if (price > parseFloat(max_price)){return alert("offer not more max price")}
        ...
        const bidData = {
            jobId, price, comment, buyer_email, email, status, category,deadline 
        }
